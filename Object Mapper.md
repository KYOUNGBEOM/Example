...
```
	...
	- Object Mapper는 직렬화를 할때 변수에 매칭되는 것이 아니라, get 메서드에 매핑됨
	  반면 역직렬화를 할때는 기본적으로 set 메서드에 매핑되고, set이 없다면 get 메서드에 매핑
 	
  	- get메서드를 매핑하고 싶지 않으면 @JsonIgnore를 사용해야함

   	- 만약 직렬화되는 변수명을 변경하고 싶거나 Json데이터 변수명과 클래스의 변수명이 맞지 않을경 @JsonProperty("") 사용
